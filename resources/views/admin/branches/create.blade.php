        ...
        <form action="{{ route("admin.branches.store") }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }} col-md-6">
             ...
             <div class="form-group {{ $errors->has('map_address') ? 'has-error' : '' }} col-md-6">
                ...
                <input type="text" id="map_address" name="map_address" value="{{@old('map_address')}}" class="form-control" required>
             ...
             <div class="form-group {{ $errors->has('stay_facility') ? 'has-error' : '' }} col-md-6">
                <label for="stay_facility">Stay Facility*</label>
                 <select id="stay_facility" name="stay_facility" class="form-control">
                    <option value="" disabled selected>Stay Facility</option>
                    <option value="Yes" {{ (@old('stay_facility') == 'Yes') ? 'selected' : '' }}>Yes</option>
                    <option value="No" {{ (@old('stay_facility') == 'No') ? 'selected' : '' }}>No</option>
               </select>
             ...
             <div class="form-group {{ $errors->has('landline_no') ? 'has-error' : '' }} col-md-6">
                ...
                <input type="text" id="landline_no" name="landline_no" value="{{@old('landline_no')}}" class="form-control" required>
               ...
               <select id="weekly_off" name="weekly_off" class="form-control">
                   <option value="">Select day</option>
                    @foreach(getdays() as $day)
                        <option value="{{$day}}" {{(@old('weekly_off') == $day) ? 'selected' : ''}}>{{$day}}</option>
                   @endforeach
               </select>
