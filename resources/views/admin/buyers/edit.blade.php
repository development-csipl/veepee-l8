            ...
            <div class="row">
                ...
                <div class="form-group {{ $errors->has('country_id') ? 'has-error' : '' }} col-md-3" style="display: none;>
                    ...
                    <option value="">Select Country</option>
                        ...
                        <option value="{{$row->id}}" {{ ((old('country_id') ?? $user->buyer->country_id) == $row->id) ? 'selected' : '' }}>{{$row->country}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('state_id') ? 'has-error' : '' }} col-md-3" >
                    ...
                    <select  id="state_id" name="state_id" class="form-control" required>
                        ...
                        @foreach($states as $row)
                            <option value="{{$row->id}}" {{($row->id == $user->buyer->state_id) ? 'selected' : ''}}>{{$row->state_name}}</option>
                        @endforeach
                    </select>               
                    ...
                    <select  id="city_id" name="city_id" class="form-control"" required>
                        <option value="">Select City</option>
                         @foreach($cities as $row)
                            <option value="{{$row->id}}" {{($row->id == $user->buyer->city_id) ? 'selected' : ''}}>{{$row->city_name}}</option>
                        @endforeach
                    </select>               
                ...
                <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }} col-md-3">
                    ...
                    <input type="text" id="address" name="address"  value="{{old('address') ?? $user->buyer->address}}" class="form-control" required>
                 ...
                 <div class="form-group {{ $errors->has('gst') ? 'has-error' : '' }} col-md-3">
                    ...
                    <input type="text" id="gst" name="gst"  value="{{old('gst') ?? $user->buyer->gst}}" class="form-control" required  maxlength="15">
                    @if($errors->has('gst'))
                        <p class="help-block">
                            {{ $errors->first('gst') }}
                        </p>
                ...
                <div class="form-group {{ $errors->has('credit_limit') ? 'has-error' : '' }} col-md-3">
                    ...
                    <input type="number" id="credit_limit" name="credit_limit"  value="{{old('credit_limit') ?? $user->buyer->credit_limit}}" class="form-control" required>
                ...
                <div class="form-group {{ $errors->has('owner_name') ? 'has-error' : '' }} col-md-3">
                    ...
                    <input type="text" id="owner_name" name="owner_name"  value="{{old('owner_name') ?? $user->buyer->owner_name}}" class="form-control" required>
                ...
                <div class="form-group {{ $errors->has('owner_contact') ? 'has-error' : '' }} col-md-3">
                    ...
                    <input type="number" id="owner_contact" name="owner_contact"  value="{{old('owner_contact') ?? $user->buyer->owner_contact}}" class="form-control" required  maxlength="10">
                ...
                <div class="form-group {{ $errors->has('order_name') ? 'has-error' : '' }} col-md-3">
                    ...
                    <input type="text" id="order_name" name="order_name"  value="{{old('order_name') ?? $user->buyer->order_name}}" class="form-control" required>
                ...
                <div class="form-group {{ $errors->has('order_contact') ? 'has-error' : '' }} col-md-3">
                    ...
                    <input type="number" id="order_contact" name="order_contact"  value="{{old('order_contact') ?? $user->buyer->order_contact}}" class="form-control" required  maxlength="10">
                ...
                <div class="form-group {{ $errors->has('sister_firm') ? 'has-error' : '' }} col-md-3">
                    ...
                    <select id="sister_firm" name="sister_firm[]" class="form-control" multiple="multiple" style="height:2rem">
                        <option value="">Select Sister Firm</option>
                        @foreach($sisterfirms as $row)
                            @if(in_array($row['id'],explode(',',$user->buyer->sister_firm))) 
                ...
                <div class="form-group {{ $errors->has('notify_email') ? 'has-error' : '' }} col-md-3" >
                    ...
                    <input type="email" id="notify_email" name="notify_email" class="form-control" value="{{old('notify_email') ?? $user->buyer->notify_email}}">
                ...
                {{--<div class="form-group {{ $errors->has('notify_sms') ? 'has-error' : '' }} col-md-3" >
                    ...
                    <input type="number" id="notify_sms" name="notify_sms" class="form-control" value="{{old('notify_sms') ?? $user->buyer->notify_sms}}"  maxlength="10">
                ...
                <div class="form-group {{ $errors->has('notify_whatsapp') ? 'has-error' : '' }} col-md-3" >
                    ...
                    <input type="number" id="notify_whatsapp" name="notify_whatsapp" class="form-control" value="{{old('notify_whatsapp') ?? $user->buyer->notify_whatsapp}}"  maxlength="10">
                ...
                <div class="form-group col-md-3" >
                    ...
                    <input type="checkbox" id="bypass" name="bypass" class="pt-2" value="1" @if($user->buyer->bypass == 1) checked @endif> Bypassed user for days & amount.
                </div>
