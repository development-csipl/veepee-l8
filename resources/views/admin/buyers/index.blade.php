...
@can('buyer_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success  float-right" href="{{ route("admin.buyers.create") }}">
                ...
                <tbody>
                    @foreach($users as $key => $value)
                        <tr data-entry-id="{{ $value->id }}">
                            ...
                            <td>
                                @can('buyer_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.buyers.show', $value->id) }}" target="_blank" disabled>
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('buyer_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.buyers.edit', $value->id) }}" disabled>
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
                               ...
                               <!--  @can('buyer_delete')
                                    <form action="{{ route('admin.buyers.destroy', $value->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
