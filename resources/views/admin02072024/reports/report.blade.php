                    ...
                    @if(@$orders->isNotEmpty())
                        ...
                        <?php  $items = DB::table('order_items')->select('name')->where('order_id',$value->id)->get();
                         ...
                         ?>
                            ...
                            <tr data-entry-id="{{ $value->id }}"  @if(!empty($value->reason) && $value->status=='Cancelled') data-toggle="tooltip" class="text-danger" title="{{$value->reason}}. | Date : {{ $value->updated_at }} | By: {{getUser($value->users_id)->name ?? '' }} | ID: {{$value->users_id}} " style="cursor:pointer" @endif >
                            <td> {{ $currentNumber++ }}</td>
                                <td > 
                                    <a >
