    ...
    <div class="card-body">
        <form action="{{ route("admin.product-categories.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('slug') ? 'has-error' : '' }}">
                ...
                <input type="text" id="slug" name="slug" class="form-control" value="{{ old('slug', isset($productCategory) ? $productCategory->slug : '') }}" required>
                @if($errors->has('slug'))
                    <p class="help-block">
                        {{ $errors->first('slug') }}
                    </p>
                @endif
                <p class="helper-block">
                    {{ trans('cruds.productCategory.fields.slug_helper') }}
                </p>
            </div>
				
				
            <div class="form-group {{ $errors->has('parentcategory') ? 'has-error' : '' }}">
                <label for="category">{{ trans('cruds.productCategory.fields.category_level') }}</label>
                <select name="parent_id" id="parent_id" class="form-control select2">
					       	<option value="0">Select Category</option>						  
                    @foreach($parentcategory as $id => $category)
                ...
                @endif
                <p class="helper-block">
                    {{ trans('cruds.productCategory.fields.category_level_helper') }}
                </p>
            </div>
				
            <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                <label for="description">{{ trans('cruds.productCategory.fields.description') }}</label>
                <textarea id="description" name="description" class="form-control ckeditor">{{ old('description', isset($productCategory) ? $productCategory->description : '') }}</textarea>
