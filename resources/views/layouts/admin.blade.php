                ...
                <ul class="navbar-nav ml-auto">
                    <li class="nav-item dropdown">
                        <a class="nav-link" data-toggle="dropdown" href="#"> 
                            <b>
                                <i class="fa fa-user" aria-hidden="true"></i> {{strtoupper(Auth::user()->name)}}
                            </b>
                        </a>
                        <div class="dropdown-menu dropdown-menu-right">
                            <a class="dropdown-item" href="{{route('change_password_form')}}">
...
<script type="text/javascript">
     $(document).on("click",".actinact",function(){
      ...
      //alert(current_ele+current_id+current_val+current_tb);
        $.ajax({
            type: "get",
             url:"{{ url('admin/statusupdate')}}/"+current_id+"/"+current_val+"/"+current_tb, 
      ...
      $(document).on("click",".islogin",function(){
        ...
        var current_val = $(this).attr('data-value');
        var current_tb = $(this).attr('data-tbl');
      //alert(current_id+'--'+current_val+'--'+current_tb);
      
        $.ajax({
            type: "get",
             url:"{{ url('admin/loginstatusupdate')}}/"+current_id+"/"+current_val+"/"+current_tb, 
            success: function( result ) {
                var obj = jQuery.parseJSON(result);
             //   alert( obj );
                $("#loginactive-"+current_id).html(obj.html);
            }
        });
      });
    ...
    $(document).ready(function(){
        $(document).on('click','.file-download',function(e){
            e.preventDefault(); 
            document.location.href= $(this).find('img').attr('src'); 
            //alert($(this).attr('data-url'));
        });
    });
    
    $(document).on('click','#donwload-sample,#export-data',function(event){
        $('#processcsv').show().text('File downloading,please wait ...');
        document.location.href = $(this).attr('data-url')+'?method='+$('#method').val()+'&numrows='+$('#numrows').val();
        $('#processcsv').show().text('File downloaded successfully !');
    });
