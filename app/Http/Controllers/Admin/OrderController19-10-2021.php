...
use App\Models\{OrderModel,TransportsModels,OrderItemModel,ItemModel,BrandModel,OrderTrackingModel,OrderDeliveryModel,SupplierBillModel,OrderGalleryModel,CourierModel,OrderCancelModel,BranchModel,OrderImageDownload};
use App\User;
...
class OrderController extends Controller{
        ...
        $headers = array(
         ...
         if($request->all()){
            ...
            if($role == 'Admin'){
                ...
                $orders = $orders1->wherein('orders.status',['Confirm','Cancelled','Completed'])->where('users.branch_id',$user->branch_id)->orderby('orders.created_at','desc');
            }elseif ($role == 'Head Office Operator') {
                $user_type = 'Head Office Operator';
                $orders = $orders1->wherein('orders.status',['Confirm','Cancelled','Completed'])->orderby('orders.created_at','desc');
            }else {
                $user_type = 'N/A';
    ...
    public function complete_bill(Request $request){
         if($request->all()){
            ...
            */
            ...
            $delivery = $delivery->orderby('created_at','asc')->paginate(20);
            /*
            $orders = OrderModel::where('branch_id',$request->branch)->get();
            $branch = array();
            foreach($orders as $row){
            ...
            */
            ...
            $user       =  Auth::user();
            $delivery   =  OrderDeliveryModel::with(['downloads'])->where('status',1)->where('veepee_invoice_number','!=',NULL)->orwhere('status',0)->orderby('created_at','asc')->paginate(20);
            return view('admin.orders.complete_bill', compact('delivery'));
        }
    }
    ...
    public function create_order(Request $request){
        if($request->all()){
            ...
            return redirect()->back()->withSuccess('Order has been created successfully & Order#:'.$order->vporder_id);
