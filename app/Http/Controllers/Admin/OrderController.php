...
class OrderController extends Controller{
    ...
    public function check_document(Request $request){
        if($request->all()){
            ...
            if($request->start_date != '' && $request->end_date != ''){  
               $users1->where('order_delivery.created_at','>=',date('Y-m-d 00:00:00',strtotime($request->start_date)))->where('order_delivery.created_at','<=',date('Y-m-d 23:59:59',strtotime($request->end_date)));
            }
            if($request->supplier_invoice != ''){
    ...
    public function update_check_delivery_status(Request $request){
        ...
        if($delivered_check){
            ...
            if($request->status == "1"){
                ...
                OrderTrackingModel::create($data1);
                $order = OrderModel::select('supplier_id')->where('id',$request->order_id)->first();
                $supplier = getUser($order->supplier_id);
                 
                $date = date('Y-m-d',strtotime($delivered_check->created_at));
                //$suppliermsg = 'Hi '.$supplier->name.' credit your '.$delivered_check->supplier_invoice.' in your '.$supplier->veepeeuser_id . ' '.$date.'. Thanks for valuable support.';
                $whatsappTemplate ='supplier1';//supp_accept
                $brlink = 'https://wholesale.veepeeonline.com/veepeeapp.html?page=Bill';
                $whatsappParameters = array(
                    array(
                        "name" => "supplier_firm_name",
                        "value" => $supplier->name,
                      ),
                      array(
                        "name" => "bill_no",
                        "value" => $delivered_check->supplier_invoice,
                      ),array(
                        "name" => "supplier_ac_no",
                        "value" => $supplier->veepeeuser_id,
                      ),array(
                        "name" => "date",
                        "value" => $date,
                      ),array(
                ...
                $supplier = getUser($order->supplier_id);
                //$suppliermsg = 'Hi '.$supplier->name.' your '.$delivered_check->supplier_invoice.' against '.$order->vporder_id.' rejected because of '. $delivered_check->reject_reason.' .Please contact to your Account Manager 917718277182.';
                $whatsappTemplate ='supplier_rejected';
                $whatsappParameters = array(
                    array(
                        "name" => "supplier_firm_name",
                        "value" => $supplier->name,
                      ...
                      array(
                        ...
                        "name" => "reason",
                        "value" => $delivered_check->reject_reason,
                      ),
                    );
            }
    ...
    public function update_check_delivery_order(Request $request){
        ...
        if($request->status == '0'){
            ...
            /* WHATSAPP NOTIFICATION ON 04-04-2024 */
                //$order = OrderModel::select('supplier_id')->where('id',$request->order_id)->first();
    ...
    public function edit_delivery_new(Request $request,$id){
        ...
        //print_r($spend_order_amount); exit();
        $order->update(['spend_order_amount' =>$spend_order_amount ]);
        if($invoice || $order_form || $supplier_bill){
            $images='';
            if($invoice){
                $images .=url('images/order_request/'.$invoice).'. ';
            }
            if($supplier_bill){
                $images .= url('images/order_request/'.$supplier_bill).'. ';
            }
            if($order_form){
                $images .=url('images/order_request/'.$order_form).'. ';
            }
            $documentattachmentlink =$images;
            $date = date('Y-m-d',strtotime($delivery->created_at));
                
                $buyer = getUser($order->buyer_id);
                //$suppliermsg = 'Hi {{buyerfirmname}} Debit Amt. {{acceptamt}} in your {{buyeraccountno}} {{date}} against {{orderfromno}} {{brand}} from {{branch}}, {{documentattachmentlink}}.For More info contact to your account managar - 918079787702';
                $whatsappTemplate ='billingpanel_buyer';
                $whatsappParameters = array(
                    array(
                        "name" => "buyerfirmname",
                        "value" => $buyer->name,
                        ...
                        "name" => "brand",
                        "value" => getBrand($order->brand_id),
                      ),array(
                        "name" => "branch",
                        "value" => getBranch($order->branch_id),
                      ),array(
                        "name" => "documentattachmentlink",
                        "value" => $documentattachmentlink,
                      ),
                    );
                //dd($whatsappParameters);
                 $buyer_sms = getBuyers($order->buyer_id);
                if($whatsappTemplate && $buyer){
                    whatsappCurl($whatsappTemplate, $buyer_sms->notify_sms, $whatsappParameters); //'8448983473' $buyer_sms->notify_sms
                }
        }
    ...
    public function create_order(Request $request){
        if($request->all()){
            ...
            $LastOrder  =    OrderConfigurationModel::select('order_increment_id')->where(['status'=>'1'])->first();
        ...
        $buyerdlt='1207163378106237130';
        $supdlt='1207163378110074486';
        $vpordid=$vpOrderId;//'VP'.$branch.'-'.$order->id;
    ...
    public function edit_order(Request $request, $id){
        ...
        if($request->all()){
            ...
            $data = array('supplier_id' => $request->supplier_id, 'transport_one_id' => $request->transport_one_id,'status' =>$request->status ?? $order->status, 'reason' => $request->reason, 'brand_id'=> $brand->id,'order_by' => $request->order_by);
