    ...
    public function CreateOrders(Request $request){
        ...
        if($user_id){
          ...
          /*Mail::send('emails.order_submitted',$data, function ($message) use($buyer) {
            ...
            /*push to supplier*/
            notifyAndroid([$user_id],'Veepee Internatonal','You order request has been set to the supplier successfully.');
    ...
    public function OrderList(Request $request){
        ...
        if($user_id){
            ...
            if($request_mode == 'supplier'){
                ...
                $orders = OrderModel::where('supplier_id',$user_id)->where('status',$order_type)->whereBetween('order_date', [$from, $to])->get();
                 if($order_type=='Rejected'){
                   $orders = OrderModel::where('supplier_id',$user_id)->where('status',$order_type)->where('updated_at','>=', $rejdate)->get();
                }
                ...
                $orders = OrderModel::where('buyer_id',$user_id)->where('status',$order_type)->whereBetween('order_date', [$from, $to])->get();
                if($order_type=='Rejected'){
                     $orders = OrderModel::where('buyer_id',$user_id)->where('status',$order_type)->where('updated_at','>=', $rejdate)->get();
...
if($pkt_cases=='' || $pkt_cases==0){
    ...
    exit;
            ...
            if($user_type =='supplier'){
                $status =  ($is_accepted==1) ? "Waiting for approval" : "Rejected";
                if($is_accepted==1){
                    ...
                    if($request->order_amount >= getSiteData()->min_order_amount){
                        ...
                        $res          = OrderModel::where(['id'=> $order_id])->update(['supplier_accept' => $is_accepted,'supplier_id' =>$supplier_id,'pkt_cases' => $pkt_cases,'order_amount' =>$order_amount,'status' => $status,'remark'=>@$request->remark]);
                ...
                $data           =  busy($userchk->veepeeuser_id);
            ...
            if($request->order_amount > $remaining){
                    return response()->json(['message'=> "Order Amount is either smaller than the minimum limit\nor greater than your Credit limit.",'status' => true,'active' => $userchk->status,'block' => $userchk->block],200);
                    exit;
                    }
            }
                $status =  !empty($is_accepted) ? "Confirm" : "Rejected";
                $res    =  OrderModel::where(['id'=> $order_id])->update(['buyer_accept' => $is_accepted,'status' => $status,'remark'=>@$request->remark]);
                // order cancel insert
    ...
    public function remainingAmount(Request $request){
        ...
        if($user_id){
            ...
            $remaining      =  ($data->amount-($Oamount-$Samount));
            //End//
            $result['remaining_amount'] = round($remaining);  
    ...
    public function statuscount($usertype, $user_id){
        ...
        $result['accepted']           = OrderModel::where('status','Confirm')->where($type,$user_id)->count();
