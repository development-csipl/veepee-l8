...
class SMSOrderNotification implements ShouldQueue {
    ...
    public function __construct($details){
        ...
        $this->link         =   url("print/".$details->order->id);//url("order/show/".$details->order->id);
        $this->bid        =   $details->order->buyer_id;
        $this->sid        =   $details->order->supplier_id;
        $this->branch       =  $details->branch->name;
        info($this->branch);
    }
    ...
    public function handle(){
        ...
        $this->buyer = 'CANCELLED, Hi '.$this->bname.' Order No '.$this->bveepeeid.' of '.$this->brand.', '.$this->branch.' given to '.$this->sname.' .Supply as per the order form ' .$this->link.'.For any query - *918079787702*';
        ...
        //send whatsapp notification
        $this->suplier  = 'CANCELLED, Hi '.$this->sname.', Order No. '.$this->vporderid.' of '.$this->brand.' , '.$this->branch.'  booked by....'.$this->bname.' .Supply as per the order form '.$this->link.' .For any query- +917718277182 ';
        $whatsappTemplate = 'veepe_ord';
        $whatsappParameters = array(
          array(
              "name" => "veepee_msg",
        ...
        //send whatsapp notification
        $this->buyer = 'CONFIRMED, Hi '.$this->bname.' Order No '.$this->vporderid.' of '.$this->brand.' , '.$this->branch.' given to '.$this->sname.' .Supply as per the order form '.$this->link.' .For any query- +918079787702 ';
        $whatsappTemplate = 'veepe_ord';
        $whatsappParameters = array(
          array(
              "name" => "veepee_msg",
        ...
        //send whatsapp notification
        $this->suplier = 'CONFIRMED, Hi '.$this->sname.' Order No '.$this->vporderid.' of '.$this->brand.' , '.$this->branch.' booked by '.$this->bname.' .Supply as per the order form '.$this->link.'.For any query- +917718277182 ';
        $whatsappTemplate = 'veepe_ord';
        $whatsappParameters = array(
          array(
              "name" => "veepee_msg",
